...
В большинстве случаев тексты получится считать, однако иногда при работе со старыми архивами могут возникнуть проблемы с кодировками. Например, все тексты в старейшей интернет-библиотеке на русском языке --- библиотеке Максима Машкова (<https://lib.ru/>) --- записаны в `KOI8-R`:
...
Пакет `gutenbergr` является API для очень старого [проекта Gutenberg](https://www.gutenberg.org/).
...
Пакет `rperseus` предоставляет API для текстов  [Perseus Digital Library](https://www.perseus.tufts.edu/hopper/), где хранятся тексты на греческом и латинском языках. 
...
Для текстов на русском языке отдельного пакета не написали, однако на сайте библиотеки [lib.ru](https://lib.ru/) они уже представлены в машиночитаемом виде, нужно всего лишь выбрать вариант отображения `txt`, полученную ссылку считать в R, указав корректную кодировку (см. @sec-encoding):
